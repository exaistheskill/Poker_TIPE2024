...
def tester_algo_iteration(i, nb_strat_1_T1, nb_strat_1_T2, nb_rgt_1_T1, nb_rgt_1_T2, nb_strat_2_T1, nb_strat_2_T2, nb_rgt_2_T1, nb_rgt_2_T2) :
    ...
    b = 0
    j = 0
    liste1 = [0 for _ in range(len(betPossible))]
    liste2 = [0 for _ in range(len(betPossible))]
    liste1_2 = [0 for _ in range(len(betPossible))]
    liste2_2 = [0 for _ in range(len(betPossible))]
    for k in range(i) :
        ...
        while jeton_P1 > 0 and jeton_P2 > 0 :
            ...
            mon_action_T1, en_action_T1 = prendre_decision_jeu_T1(j,PP1,nb_rgt_1_T1, nb_strat_1_T1,PP2, nb_rgt_2_T1, nb_strat_2_T1,  jeton_P1, jeton_P2)
            
            #en_action_T1, mon_action_T1 = prendre_decision_jeu_T1(j+1,PP2, nb_rgt_2_T1, nb_strat_2_T1, PP1,nb_rgt_1_T1, nb_strat_1_T1,  jeton_P2, jeton_P1)
            liste1[en_action_T1] += 1
            liste2[mon_action_T1] += 1
            mon_action_T1,en_action_T1, is_fold_P1, is_fold_P2, partie_continue =jouer_partie_T1(j,P1, mon_action_T1, nb_strat_1_T1, P2, en_action_T1, nb_strat_2_T1, jeton_P1, jeton_P2)
            #en_action_T1, mon_action_T1, is_fold_P2, is_fold_P1, partie_continue =jouer_partie_T1(j+1, P2, en_action_T1, nb_strat_2_T1,P1, mon_action_T1, nb_strat_1_T1, jeton_P2, jeton_P1)
            #le Tour 1 est fini
            ...
            if partie_continue == True :
                ...
                mon_action_T2, en_action_T2 = prendre_decision_jeu_T2(j,PP1, ma_combi, nb_strat_1_T2,  nb_rgt_1_T2, PP2,en_combi, nb_strat_2_T2, nb_rgt_2_T2, jeton_encr_P1, jeton_encr_P2)
                #en_action_T2, mon_action_T2 = prendre_decision_jeu_T2(j+1, PP2,en_combi, nb_strat_2_T2, nb_rgt_2_T2, PP1, ma_combi, nb_strat_1_T2,  nb_rgt_1_T2,jeton_encr_P2, jeton_encr_P1)
                recomp_P1, recomp_P2 = jouer_partie_T2(j,P1,ma_combi, mon_action_T2, mon_action_T1, nb_strat_1_T2, P2,en_combi, en_action_T2, en_action_T1, nb_strat_2_T2 ,flop, jeton_encr_P1, jeton_encr_P2)
                #recomp_P2, recomp_P1 = jouer_partie_T2(j+1,P2,en_combi, en_action_T2, en_action_T1, nb_strat_2_T2 ,P1,ma_combi, mon_action_T2, mon_action_T1, nb_strat_1_T2, flop, jeton_encr_P2, jeton_encr_P1)
                liste1_2[mon_action_T2] += 1
                liste2_2[en_action_T2] += 1
        ...
        if jeton_P2 <= 0 :
            c += 1
    print(c,b)
    return liste1,liste2,liste1_2, liste2_2
...
def test_2_algo(fich1, fich2,i) :
    liss1,liss2,liss3,liss4 = np.array(test_entre_2_strat(i,fich1,fich2))
    plt.figure()
    plt.title(f"{fich1} vs {fich2}")
    X = np.linspace(0,len(liss1)-1, len(liss1))
    plt.plot(X, liss1, "o", c= "blue", label = "P1_T1")
    plt.plot(X, liss2, "o", c = "red", label = "P2_T1")
    plt.plot(X, liss3, "o", c = 'yellow', label = "P1_T2")
    plt.plot(X, liss4, "o", c = "green", label = "P2_T2")
    plt.legend()
    plt.show()

def norme_2_algo(fich1, fich2) :
    N1,N2,N3,N4 = prendre_strat_de_fichier(fich1)
    M1,M2,M3,M4 = prendre_strat_de_fichier(fich2)
    somme2 = 0
    somme4 = 0
    for i in combi_main :
            ...
            for k in combinaison :
                for l in range(1,len(carte_possible)+1) :
                    for n in range(1,len(carte_possible)+1) :
                        for m in [False, True] :
                            for o in range(nombreActions) :
                                somme2 += (N2[(conv_carte_str(i), (k,l,n,m),j)][o] - M2[(conv_carte_str(i), (k,l,n,m),j)][o])**2
                                somme4 += (N4[(conv_carte_str(i), (k,l,n,m),j)][o] - M4[(conv_carte_str(i), (k,l,n,m),j)][o])**2
    somme1 = 0
    somme3= 0
    for i in combi_main :
        for j in range(-1,nombreActions) :
            for o in range(nombreActions) :
                somme1 += (N1[(conv_carte_str(i),j)][o]-M1[(conv_carte_str(i),j)][o])**2
                somme3 += (N3[(conv_carte_str(i),j)][o]-M3[(conv_carte_str(i),j)][o])**2
    somme1, somme2, somme3, somme4 = np.sqrt(somme1), np.sqrt(somme2), np.sqrt(somme3), np.sqrt(somme4)
    return (somme1 + somme2 + somme3 + somme4)/4
