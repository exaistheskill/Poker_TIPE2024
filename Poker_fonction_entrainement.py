...
for i in combi_main :
    for j in range(-1,nombreActions) :
        for k in combinaison :
            for l in range(1,6) :
                for m in [False, True] :
                    nombre_regret_T2[(conv_carte_str(i), (k,l,m),j)] = np.zeros(nombreActions)
                    en_nombre_regret_T2[(conv_carte_str(i), (k,l,m),j)] = np.zeros(nombreActions)
                    nombreStrategies_T2[(conv_carte_str(i), (k,l,m),j)] = np.zeros(nombreActions)
                    en_nombreStrategies_T2[(conv_carte_str(i), (k,l,m),j)] = np.zeros(nombreActions)
for i in combi_main :
    for j in range(-1,nombreActions) :
...
def prendre_decision_jeu_T1_print(j,ma_main, mon_nombre_reg, mon_nombre_strat, en_main, en_nombre_reg,en_nombre_strat, jeton_P1, jeton_P2) :
    ...
    if j%2 == 0 :
        ...
        for j in range(nombreActions) :
            mon_nombre_strat[(PP1,-1)][j] += ma_strategie_T1[j]
            en_nombre_strat[(PP2,mon_action_T1)][j] += en_strategie_T1[j]
    if j%2 == 1 :
        ...
        for j in range(nombreActions) :
            mon_nombre_strat[(PP1,en_action_T1)][j] += ma_strategie_T1[j]
            en_nombre_strat[(PP2,-1)][j] += en_strategie_T1[j]
